...
impl CiffToBmp {
    ...
    pub fn bsize(&mut self, bsize: Vec<usize>) -> &mut Self {
    ...
    pub fn to_bmp(&self) -> Result<()> {
        ...
        let bsize = self
            .bsize
            .clone()
            .ok_or_else(|| anyhow!("bsize undefined"))?;
...
fn convert_to_bmp(
    ...
    // we want bsize to have the smallest blocks last
    let mut bsize = ref_bsize.clone(); // need to clone to fix weird mut problem
    bsize.sort_unstable();
    bsize.reverse();
    eprintln!("Building index with blocks: {:?}", bsize);
    let mut builder: IndexBuilder;
    {
        ...
        builder = IndexBuilder::new(header.num_documents as usize, bsize.clone());
    ...
    let smallest_bsize = bsize[bsize.len() - 1];
    let b_forward_index = crate::index::forward_index::fwd2bfwd(&forward_index, smallest_bsize);
